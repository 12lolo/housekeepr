...
<body @isset($isNewHotel) data-new-hotel="{{ $isNewHotel ? 'true' : 'false' }}" @endisset @isset($stats) data-total-rooms="{{ $stats['total_rooms'] ?? 0 }}" @endisset>
        ...
        <div class="neu-modal neu-modal-sm">
            ...
            <div class="neu-modal-body">
                ...
                <div style="text-align: center; margin-bottom: 2rem;">
                    ...
                    <h4 style="margin: 0 0 0.25rem 0; font-size: 1.25rem;">{{ auth()->user()->name }}</h4>
    ...
    <script>
        ...
        // Tour reset function - for testing
        window.resetTour = function() {
            localStorage.removeItem('housekeepr_tour_completed');
            console.log('✅ Tour reset! Refreshing page...');
            setTimeout(() => location.reload(), 500);
        };


        // Show tour reset button if tour has been completed
        if (localStorage.getItem('housekeepr_tour_completed')) {
            const resetBtn = document.createElement('button');
            resetBtn.innerHTML = '🔄 Reset Tour';
            ...
            resetBtn.onclick = () => {
                if (confirm('Reset de tour? De pagina wordt ververst.')) {
                    window.resetTour();
                }
            };
