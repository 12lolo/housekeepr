...
@section('page-title', 'Nieuw Issue')
...
@section('content')
    <div class="neu-widget">
        <div class="widget-header">
            <h2>Nieuw Issue Melden</h2>
            <a href="{{ route('owner.issues.index') }}" class="neu-button-secondary">
        ...
        <div class="widget-body">
            <form action="{{ route('owner.issues.store') }}" method="POST">
                @csrf
                ...
                <div class="neu-form-group">
                    ...
                    >
                        ...
                        <option value="geen_haast" {{ old('impact') == 'geen_haast' ? 'selected' : '' }}>Geen Haast</option>
                        <option value="graag_snel" {{ old('impact') == 'graag_snel' ? 'selected' : '' }}>Graag Snel</option>
                        <option value="kan_niet_gebruikt" {{ old('impact') == 'kan_niet_gebruikt' ? 'selected' : '' }}>Kamer Kan Niet Gebruikt Worden (BLOKKEREND)</option>
                    </select>
                    @error('impact')
                        <span class="neu-error">{{ $message }}</span>
                    @enderror
                    <small class="neu-hint">Let op: "Kamer Kan Niet Gebruikt Worden" blokkeert planning voor deze kamer.</small>
                </div>

                <div class="neu-form-group">
                    <label for="note" class="neu-label">Beschrijving</label>
                    <textarea
                        id="note"
                        name="note"
                ...
                <div class="form-actions">
                    ...
                    <a href="{{ route('owner.issues.index') }}" class="neu-button-secondary">Annuleren</a>
                </div>
            </form>
        </div>
    </div>
