...
@section('page-title', 'Nieuwe Schoonmaker')
...
@section('content')
    <div class="neu-widget">
        <div class="widget-header">
            <h2>Nieuwe Schoonmaker Toevoegen</h2>
            <a href="{{ route('owner.cleaners.index') }}" class="neu-button-secondary">
        ...
        <div class="widget-body">
            <form action="{{ route('owner.cleaners.store') }}" method="POST">
                @csrf

                <div class="neu-form-group">
                    <label for="name" class="neu-label">Naam Schoonmaker</label>
                    <input
                        type="text"
                        id="name"
                        name="name"
                ...
                <div class="neu-form-group">
                    ...
                    <input
                        ...
                        value="{{ old('email') }}"
                        placeholder="schoonmaker@hotel.nl"
                        required
                    >
                    @error('email')
                        <span class="neu-error">{{ $message }}</span>
                    @enderror
                    <small class="neu-hint">Een tijdelijk wachtwoord wordt automatisch gegenereerd. Noteer dit zorgvuldig!</small>
                </div>
                ...
                <div class="form-actions">
                    <button type="submit" class="neu-button-primary">
                        ...
                        Schoonmaker Toevoegen
                    </button>
                    <a href="{{ route('owner.cleaners.index') }}" class="neu-button-secondary">Annuleren</a>
                </div>
            </form>
        </div>
    </div>
